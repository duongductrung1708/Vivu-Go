...
export async function POST(request: NextRequest) {
  ...
  if (!genAI) {
    return NextResponse.json({ error: "Failed to initialize Gemini AI client" }, { status: 500 });
  }
      ...
      return NextResponse.json(
        {
          ...
          details:
            lastError?.message ||
            "Could not initialize Gemini model. Please check your API key and model availability.",
        },
        { status: 500 },
      );
    }
...
Hãy trả lời bằng tiếng Việt, thân thiện và hữu ích.`;
    ...
    if (context) {
      ...
      if (context.days && context.days.length > 0) {
        ...
        context.days.forEach(
          (day: { date: string; places: Array<{ name: string; category: string }> }) => {
    ...
    if (errorMessage.includes("API_KEY") || errorMessage.includes("api key")) {
      return NextResponse.json(
        {
          error:
            "Invalid or missing Gemini API key. Please check your GEMINI_API_KEY in .env.local",
        },
        { status: 500 },
      );
    }
