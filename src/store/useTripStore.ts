...
type TripStore = {
  ...
  ) => Promise<{ totalDistance: number } | null>;
...
export const useTripStore = create<TripStore>()((set, get) => ({
  ...
  setConfig: ({ name, startDate, endDate, peopleCount, totalBudget }) => {
    set((state) => {
      ...
      for (
        ...
        const existing = state.trip.days.find((day) => day.date === iso);
        days.push(
          existing ?? {
            id: `day-${iso}`,
            date: iso,
      ...
      trip: {
        ...
        days: state.trip.days.map((day) =>
          day.id === dayId
            ? {
                ...day,
                places: day.places.filter((placeItem) => placeItem.id !== placeId),
              }
            : day,
        ),
      },
  ...
  optimizeRoute: async (dayId, profile = "driving") => {
    ...
    set((state) => ({
      trip: {
        ...state.trip,
        days: state.trip.days.map((tripDay) =>
          tripDay.id === dayId ? { ...tripDay, places: result.optimizedPlaces } : tripDay,
        ),
      },
    }));
  ...
  getSelectedDay: () => get().trip.days.find((day) => day.id === get().selectedDayId),
  ...
  getDayCost: (dayId) => {
    ...
    return day.places.reduce((sum, place) => sum + (place.estimatedCost || 0), 0);
  },
